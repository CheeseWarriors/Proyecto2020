...
namespace PROYECTO_GUERRA
{
    public partial class Registro : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (textBoxCI.Text.Length == 8)// 1 - ¿La CI tiene 8 dígitos?
                                           //NOTA: NO CHEQUEA si los datos introducidos son LETRAS o NÚMEROS

            {
                if (textBoxNomUsu.Text.Length >= 8)      // 2 - ¿El nombre de usuario tiene al menos 8 caracteres..

                {
                    if (textBoxNomUsu.Text.Length <= 12)  // ... y no más de 12 caracteres?
                    /* NOTA: El TextBox NO PERMITE textos más largos que 12 caracteres
                     * ya que se modificó su propiedad MaxLength
                     */

                    {
                        if (textBoxContrasena.Text.Length >= 8)  //3 - ¿La contraseña tiene al menos 8 caracteres...
                        /* NOTA: NO CHEQUEA la complejidad de la contraseña **/
                        {
                            if (textBoxContrasena.Text.Length <= 15)  //...y no más de 15 caracteres?
                                                                    // NOTA: ver NOTA anterior
                            {
                                if (textBoxContrasena.Text.Equals(textBoxVContrasena.Text))  //4 - ¿La contraseña y su repetición coinciden?
                                {
                                    ...
                                    string usuario = textBoxNomUsu.Text;
